  ...
  async function renderLeaderboard() {
    ...
    try {
      ...
      if (leaderboardTab === 'global') {
        leaderboardContent.innerHTML = `${periodButtons()}<p class="social-note social-error">Global leaderboards and stats are currently down because they lagged the server. Leaderboards will be back in order soon.</p><p class="social-note">Global rank is the average of all category ranks. Only Discord-linked players appear here.</p><div class="leaderboard-list">${data.rows.length ? data.rows.map((row) => `<div class="leader-row"><strong>#${row.globalRank}</strong><span>${escape(row.username)}</span><span>${row.guild ? escape(row.guild) : 'No guild'}</span></div>`).join('') : '<p class="social-note">No Discord-linked players have stats for this period yet.</p>'}</div>`;
      } else if (!data.me) {
        ...
        leaderboardContent.innerHTML = `<p class="social-note social-error">Global leaderboards and stats are currently down because they lagged the server. Leaderboards will be back in order soon.</p><p class="social-note">Your private standings for ${periods.find(([id]) => id === period)[1]}. Global rank: #${data.me.globalRank ?? '—'}.</p><div class="leaderboard-list"><div class="leader-row stats-row"><strong>Category</strong><span>Your total</span><span>Your rank</span></div>${metrics.map(([label, key, suffix]) => {
  ...
  async function renderGuild() {
    ...
    try {
      ...
      if (guildTab === 'leaderboard') {
        ...
        guildContent.innerHTML = `<p class="social-note social-error">Global leaderboards and stats are currently down because they lagged the server. Leaderboards will be back in order soon.</p><p class="social-note">Guild score is based on the average member leaderboard score.</p><div class="leaderboard-list">${data.guilds.length ? data.guilds.map((guild, index) => `<div class="leader-row"><strong>#${index + 1}</strong><span>[${escape(guild.tag)}] ${escape(guild.name)}</span><span>${number(guild.score)} score · ${guild.members} members</span></div>`).join('') : '<p class="social-note">There are no approved guilds yet.</p>'}</div>`;
