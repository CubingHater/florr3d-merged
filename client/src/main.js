...
import { preloadPetalModels } from './petalmodels.js';
import { makeMinimap } from './minimap.js';
import { setupMobileControls, joyWorldOffset } from './mobile.js';
import { CHAT_MAX_LEN, NAME_MAX_LEN } from '../../shared/config.js';
...
preloadPetalModels(Object.keys(PETAL_TYPES));
...
Promise.all([
  ...
  if (new URLSearchParams(location.search).has('merge')) {
    fetch(apiUrl('/auth/merge'), { credentials: 'include' })
  ...
  fetch(apiUrl('/auth/me'), { credentials: 'include' }).then((r) => (r.ok ? r.json() : null)).then((me) => {
    if (!me) return;
    if (!me.loggedIn) {
      if (me.loginEnabled) {
        loginBtn.href = apiUrl('/auth/discord');
    ...
    logout.id = 'logoutbtn';
    logout.href = apiUrl('/auth/logout');
  ...
  game.input.on('c', () => game.ui.toggleCrafting());
  ...
  for (let i = 1; i <= 10; i++) {
    ...
    game.input.on(key, () => {
      if (i > (game.entities.state?.petals?.primary?.length || 5)) return;
  ...
  function loop() {
    ...
    game.ui.updateCraftAnim(dt);

    updateCamera(dt, game.entities.playerPos(), game.fpsMode ? game.input.look : null);
    game.arrows.update();
    
    // Check if player has drop radar (lightbulb)
    const state = game.entities.state;
    let hasDropRadar = false;
    if (state?.petals?.primary) {
      for (const slot of state.petals.primary) {
        if (slot && petalStat(slot, 'dropRadar', false)) {
          hasDropRadar = true;
          break;
        }
      }
    ...
    updateMinimap(game.entities.playerPos(), drops, hasDropRadar);
