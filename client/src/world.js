...
import { makeWalls, cullTerrainChunks } from './walls.js';
import { makeGrass } from './grass.js';
import tileUrl from '../assets/tile.svg';

const TILE_WORLD_SIZE = 10;

function loadTileTexture(renderer) {
  return new Promise((resolve) => {
    const texture = new THREE.TextureLoader().load(tileUrl, (tx) => {
      tx.wrapS = THREE.RepeatWrapping;
      tx.wrapT = THREE.RepeatWrapping;
      tx.colorSpace = THREE.SRGBColorSpace;
      tx.anisotropy = renderer.capabilities.getMaxAnisotropy();
      tx.repeat.set(1 / TILE_WORLD_SIZE, 1 / TILE_WORLD_SIZE);
      tx.needsUpdate = true;
      resolve(tx);
    }, undefined, () => {
      resolve(null);
...
export function createWorld(container) {
  ...
  renderer.setPixelRatio(highQ ? Math.min(window.devicePixelRatio, 2) : 1);
  ...
  if (ultra) {
    // Ultra grass texture temporarily disabled - grass_color.jpg missing
  ...
  function updateCamera(dt, focus, look = null) {
    ...
    if (updateT%30==0) cullTerrainChunks(camera.position);
    updateT++;
  }
