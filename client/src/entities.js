...
import { makePlayerMesh, makeMobMesh, makeHealthBar, makePetalMesh, makeDropMesh, makeMissileMesh, attachFlowerWings } from './models.js';
...
export class EntitySync {
  constructor(game) {
    ...
    this.playerMesh = makePlayerMesh('', PLAYER_RADIUS);
  ...
  setPlayerModel(model) {
    model ||= '';
    if (model === this.playerModel) return;
    const old = this.playerMesh;
    this.game.scene.remove(old);
    disposeObject3D(old);
    this.playerMesh = makePlayerMesh(model, PLAYER_RADIUS);
    this.playerMesh.position.copy(this.playerTarget);
    if (!model) attachFlowerWings(this.playerMesh);
  ...
  apply(state) {
    ...
    if (state.player) {
      this.setPlayerModel(state.player.model);
    ...
    for (const pl of state.players) {
      for (const inst of pl.petals.instances) {
        const item = { ...inst, ox: pl.x, oy: pl.y || 0, oz: pl.z, ownerDead: pl.dead };
        // Orbit petals should never be more than a few units from their
        // owner. A delayed/reused network record used to leave them stranded
        // at an old world position; snap that invalid state back immediately.
        if (item.alive && Math.hypot(item.x - pl.x, item.z - pl.z) > 12) {
          item.x = pl.x;
          item.z = pl.z;
        }
        petalItems.push(item);
      }
    }
  ...
  handleEvent(ev) {
    ...
    } else if (ev.e === 'jobapplication') { // of COURSE cubing would add this
      if (ev.id === this.state.you) {
  ...
  createPlayer(p) {
    const mesh = makePlayerMesh(p.model, PLAYER_RADIUS);
    mesh.position.set(p.x, PLAYER_RADIUS + (p.y || 0), p.z);
    if (!p.model) attachFlowerWings(mesh);
    ...
    const view = {
      id: p.id, mesh, hpBar, nameSprite: null, name: undefined, model: p.model || '',
  ...
  createMob(m) {
    ...
    if (!def) {
      console.error('Unknown mob type:', m.type, m);
      return null;
    }
  ...
  createPetal(p) {
    const size = petalStat(p, 'radius') * (1 + p.rarity * 0.12);
