import { ARENA_HALF, TILE_SIZE, MAP_TILES, MAP_WALLS, RARITIES } from '../../shared/config.js';
...
const WALL_COLORS = { dirtWall: '#54402a', stoneWall: '#000000', centraliaWall: '#6e6e75' };
...
export function makeMinimap(parent) {
  ...
  return (pos, drops = [], hasDropRadar = false) => {
    ctx.drawImage(bg, 0, 0, canvas.width, canvas.height);

    // Draw drops as dots if player has drop radar (lightbulb)
    if (hasDropRadar && drops.length > 0) {
      for (const drop of drops) {
        const dx = ((drop.x + ARENA_HALF) / (2 * ARENA_HALF)) * canvas.width;
        const dz = ((drop.z + ARENA_HALF) / (2 * ARENA_HALF)) * canvas.height;
        ctx.beginPath();
        ctx.arc(dx, dz, Math.max(2, scale * 0.5), 0, Math.PI * 2);

        // Use rarity color for Ultra+ drops, yellow for others
        const rarity = RARITIES[drop.rarity];
        if (rarity && drop.rarity >= 7) { // Ultra (7) and above
