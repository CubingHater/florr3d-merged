...
import { hasPetalModel, swapInPetalModel } from './petalmodels.js';
...
export function makePlayerMesh(model, radius = 1.1) {
  const flower = makeFlower(radius);
  if (model && hasMobModel(model)) swapInMobModel(flower, model, radius);
  return flower;
}
...
function makeScorpionMob(radius) {
  const group = new THREE.Group();
...
export function makeMobMesh(type, radius) {
  ...
  else if (type === 'ladybug' || type === 'shinyladybug' || type === 'jungleladybug') group = makeLadybugMob(radius);
  ...
  else if (type === 'scorpion') group = makeScorpionMob(radius);
  else if (hasMobModel(type)) {
    // No bespoke procedural shape for this one — just an empty placeholder
    // that gets swapped for its GLB model below (e.g. beetle, cactus, bush,
    // leafbug and its variants, nazarbeetle, firefly).
  ...
  if (hasMobModel(type)) {
    swapInMobModel(group, type, radius, (g, inst, r) => {
      // if (type == 'ladybug') { // spider
      //   const box = new THREE.Box3().setFromObject(inst);
      //   const legs = new THREE.Group();
      //   // legs.rotation.y = 0;//-Math.PI;
      //   // legs.rotation.x = Math.PI;
      //   //legs.position.set(0.192, 0.048, -0.065);
      //   //legs.scale.set(0.166, 0.111, 0.063);
      //   //legs.position.set((box.min.x + box.max.x) / 2, (box.min.y + box.max.x) / 2, (box.min.z + box.max.z) / 2);
      //   swapInMobModel(legs, 'spiderlegs', r, null);
      //   g.add(legs);
      // }
      if (type == 'hornet') {
        const box = new THREE.Box3().setFromObject(inst);
...
export function makePetalMesh(type, radius) {
  if (hasPetalModel(type)) {
    const group = new THREE.Group();
    enableShadows(group, { cast: true, receive: false });
    swapInPetalModel(group, type, radius);
    return group;
  }
  ...
  let outlined = false;
  if (type === 'rockPetal') {
    const mat = toonMat(def.color(0));
    mat.flatShading = true;
    mesh = new THREE.Mesh(makeRockGeometry(radius, 0.2), mat);
  } else if (type === 'goldenleaf') {
    ...
    const darkGreen = '#CDB660';
    mesh = new THREE.Mesh(geo, toonMat(def.color(0)));
    ...
    outlined = true;
  } else if (type === 'holyleaf') {
    const L = radius * 1.15, W = radius * 0.7, depth = radius * 0.16;
    const geo = sharedGeo(`holyleaf-${radius}`, () => {
      const shape = new THREE.Shape();
      shape.moveTo(0, L);
      shape.quadraticCurveTo(W * 1.05, L * 0.25, W * 0.9, -L * 0.3);
      shape.quadraticCurveTo(W * 0.6, -L * 0.95, 0, -L);
    ...
    const darkGreen = '#5d6066';
    const mat = toonMat('#868A8F');
    mesh = new THREE.Mesh(geo, mat);
    mesh.material.color.set('#868A8F');
    const ribGeo = sharedGeo(`holyleaf-rib-${radius}`, () =>
      new THREE.BoxGeometry(radius * 0.09, radius * 0.05, L * 1.5));
    const rib = new THREE.Mesh(ribGeo, toonMat(darkGreen));
    rib.position.set(0, depth * 0.5 + radius * 0.02, -L * 0.1);
    mesh.add(rib);
    const stemGeo = sharedGeo(`holyleaf-stem-${radius}`, () =>
      new THREE.BoxGeometry(radius * 0.12, radius * 0.12, radius * 0.45));
    const stem = new THREE.Mesh(stemGeo, toonMat(darkGreen));
    stem.position.set(0, 0, -(L + radius * 0.15));
    mesh.add(stem);
    addOutline(mesh, 0.15, darkGreen);
    outlined = true;
  } else if (type === 'holyrice') {
    const geo = sharedGeo(`holyrice-${radius}`, () => {
    ...
    mesh.scale.y = 0.85;
    addOutline(mesh, 0.2, '#5d6066');
    outlined = true;
  } else if (type === 'holycorn') {
    const geo = sharedGeo(`holycorn-${radius}`, () => {
    ...
    mesh = new THREE.Mesh(geo, mat);
    mesh.material.color.set('#868A8F');
    addOutline(mesh, 0.14, '#5d6066');
    outlined = true;
  } else if (type === 'holywing') {
    const geo = sharedGeo(`holywing-${radius}`, () => {
      const R = radius * 1.15;
      const shape = new THREE.Shape();
    ...
    mesh = new THREE.Mesh(geo, mat);
    mesh.material.color.set('#868A8F');
    addOutline(mesh, 0.15, '#5d6066');
    outlined = true;
  } else if (type === 'stinger') {
    mesh = new THREE.Mesh(new THREE.ConeGeometry(radius * 0.9, radius * 2, 3), toonMat(def.color(0)));
    mesh.rotation.x = Math.PI / 2;
  } else if (type === 'missile') {
    mesh = new THREE.Mesh(new THREE.ConeGeometry(radius * 1.1, radius * 2.3, 3), toonMat(def.color(0)));
    ...
    mesh = new THREE.Mesh(geo, new THREE.MeshPhysicalMaterial({
      color: def.color(0), transparent: true, opacity: 0.5, roughness: 0.1,
    }));
  } else if (type === 'rice') {
    ...
    mesh = new THREE.Mesh(geo, new THREE.MeshPhysicalMaterial({
      color: def.color(0), transparent: true, opacity: 0.4, roughness: 0.05,
    }));
    ...
    mesh = new THREE.Mesh(new THREE.SphereGeometry(radius, 16, 12), toonMat(def.color(0)));
...
export function makeDropMesh(type, rarityIdx) {
  ...
  const petal = makePetalMesh(type, PETAL_TYPES[type].radius(rarityIdx) * 1.5);
