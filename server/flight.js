import { FLIGHT, mapHeightAtPoint } from '../shared/config.js';
...
export function updateFlight(p, dt) { // for some reason this is called p instead of player
  const input = p.input;
  const v = p.flightVel;
  ...
  let minFloor = mapHeightAtPoint(p.pos.x, p.pos.z);
  let airborne = p.pos.y > minFloor + 0.1;
  ...
  minFloor = mapHeightAtPoint(p.pos.x, p.pos.z);
  if (p.pos.y <= minFloor) {
    p.pos.y = minFloor;
    if (v.y < 0) v.y = 0;
  }
