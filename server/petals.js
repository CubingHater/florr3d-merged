...
import { PETAL_TYPES, RARITIES, ARENA_HALF, wallTopAt, MOB_TYPES } from '../shared/config.js';
...
export class PetalManager {
  ...
  makeInstances(slot, slotIdx, total, startPosIdx, readyNow) {
    ...
    const slow = def.slow || 0;
      out.push({
        ...
        minusarmor: def.minusarmor || 0,
        flowerarmor: flowerArmor,
        flowerhealth: flowerHealth,
        yinyang: def.yinyang || false,
        petalsummon: def.petalsummon || null,
      });
    }
  ...
  getGoldenLeafReloadMultiplier() {
    ...
    // Common = 0%, Unusual = settings.goldenLeafPercentPerRarity, Rare =
    // 2x that, and so on. Multiple Golden Leafs stack, but no reload may be
  ...
  update(dt) {
    ...
    this.rot += BASE_ROT_SPEED * this.rotFactor * rotDirection * dt;
