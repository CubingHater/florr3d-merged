...
export async function handleAuth(req, res) {
  ...
  switch (url.pathname) {
    case '/auth/discord': {
      ...
      // Store the redirect URL in a cookie so we can return after Discord login
      const redirectBack = url.searchParams.get('redirect') || '/admin/public/admin.html';
      res.appendHeader('Set-Cookie', `auth_redirect=${encodeURIComponent(redirectBack)}; Path=/; Max-Age=600; HttpOnly; ${cookieSameSite}`);
      // NOTE: do NOT add prompt: 'none' here. Discord only skips the consent
      // screen for prompt=none if the user already authorized this app before
      // — for anyone who hasn't (i.e. most normal players), Discord returns
      // an error instead of showing the consent screen, so they never get a
      // `code` back and login silently fails. Leaving prompt unset makes
      // Discord show the consent screen when needed and skip it when not,
      // which is the correct behavior for a login flow.
    ...
    case '/auth/callback': {
      ...
      // Get the redirect URL from the cookie (set by /auth/discord)
      const cookies = parseCookies(req.headers.cookie);
      const redirectBack = cookies.auth_redirect ? decodeURIComponent(cookies.auth_redirect) : frontendHome();
      // Clear the redirect cookie
      res.appendHeader('Set-Cookie', `auth_redirect=; Path=/; Max-Age=0; HttpOnly; ${cookieSameSite}`);
