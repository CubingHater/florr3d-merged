import {
  RARITIES, VIEW_RADIUS, PITCH_LIMIT,
  CHAT_MAX_LEN, NAME_MAX_LEN, stripNonAscii, PLAYER_MODEL_IDS,
} from '../shared/config.js';
import { PETAL_TYPES, petalStat, CRAFT_CHANCES } from '../shared/petals.js';
import { censorName, censorMessage } from './censor.js';
import { craftPetals } from './crafting.js';
...
const FAILED_CRAFT_POINTS = [0, 1, 2, 3, 4, 5, 6, 25, 75];
const SUCCESS_CRAFT_POINTS = [0, 5, 10, 15, 20, 25, 30, 70, 210];

export class World {
  // The database hook is injected by the real websocket server. Keeping it
  // optional lets the local/browser worker use World without bundling SQLite.
  constructor({ removePersistedEternalPharCrowns = null } = {}) {
    this.time = 0;
    this.events = [];
    this.externalEvents = []; // Persistent events for external API
    this.players = new Map();
    this.mobs = new MobManager(this);
    this.drops = new DropManager(this);
    this.removePersistedEternalPharCrowns = removePersistedEternalPharCrowns;
    this.spatialGrid = new SpatialGrid(32); // 32-unit grid cells for spatial partitioning
    this.inventoryCache = new Map(); // Cache for serialized inventory to avoid repeated JSON operations
  }
  ...
  removePlayer(id) {
    ...
    // Clean up inventory cache to prevent memory leaks
    this.inventoryCache.delete(id);
  }

  awardEternalPharCrown(winner) {
    if (!winner) return;
    let stolen = this.removePersistedEternalPharCrowns?.() > 0;
    for (const player of this.players.values()) {
      const key = 'pharcrown:9';
      const legacyKey = 'crown:9';
      if (player.inventory.has(key) || player.inventory.has(legacyKey)) stolen = true;
      player.inventory.delete(key);
      player.inventory.delete(legacyKey);
      let changed = false;
      for (const row of [player.petals.primary, player.petals.secondary]) {
        for (let i = 0; i < row.length; i++) {
          if (row[i] && (row[i].type === 'pharcrown' || row[i].type === 'crown') && row[i].rarity === 9) { row[i] = null; changed = true; }
        }
      }
      if (changed) player.petals.rebuildAll(false);
      player.invDirty = true;
    }
    winner.addToInventory('pharcrown', 9, true);
    const message = stolen
      ? `The eternal crown has been stolen by ${winner.name}`
      : `The eternal crown has been claimed by ${winner.name}`;
    this.events.push({ e: 'toast', text: message });
    this.externalEvents.push({ e: 'toast', text: message });
  }
  ...
  handle(playerId, msg) {
    ...
    switch (msg.t) {
      ...
      case 'swapSlot': {
        const i = msg.i;
        if (Number.isInteger(i) && i >= 0 && i < player.petals.primary.length) player.petals.swapSlot(i);
        break;
      }
      case 'swapRows':
      ...
      case 'equip': {
        const { row, i, key } = msg;
        if ((row !== 'primary' && row !== 'secondary') || !Number.isInteger(i) || i < 0 || i >= player.petals.primary.length) return;
        ...
        player.refreshLoadoutSlots();
        break;
      }
      case 'setModel': {
        const model = typeof msg.model === 'string' ? msg.model : '';
        // '' is the default flower look; anything else must be a known
        // cosmetic id, same allowlist the admin panel already uses.
        if (model !== '' && !PLAYER_MODEL_IDS.includes(model)) return;
        player.model = model;
        break;
      }
      case 'chat': {
        ...
        // Don't add chat events to externalEvents - only spawn, kill, and drop events
        break;
      }
      case 'craftAtt': {
        // Never trust the client's petal count directly — validate against
        // what the player actually owns, and reject anything that isn't a
        // real, craftable petal/rarity. Without this, a malicious client
        // could send an arbitrary count and receive free upgraded petals.
        const type = msg.type;
        const rarity = Number(msg.rarity);
        const def = PETAL_TYPES[type];
        if (!def || !Number.isInteger(rarity) || !RARITIES[rarity]) break;
        if (def.uncraftable) break;
        if (rarity < 0 || rarity >= CRAFT_CHANCES.length) break;
        const owned = player.inventory.get(`${type}:${rarity}`) || 0;
        const count = Math.min(Math.floor(Number(msg.count) || 0), owned);
        if (count < 3) break;
        const { lose, gain, attempts } = craftPetals({ type, rarity, count });
        for (let i = 0; i < lose.count; i++) player.takeFromInventory(`${lose.type}:${lose.rarity}`);
        for (let i = 0; i < gain.count; i++) player.addToInventory(gain.type, gain.rarity, true);
  ...
  buildTick(spectators = []) {
    ...
    for (const p of players) {
      ...
      if (p.invDirty) {
        // Use cached inventory serialization to avoid repeated array operations
        const cacheKey = p.id;
        if (!this.inventoryCache.has(cacheKey) || p.invDirty) {
          this.inventoryCache.set(cacheKey, [...p.inventory.entries()]);
        }
        view.inventory = this.inventoryCache.get(cacheKey);
        p.invDirty = false;
      }
      views.set(p.id, view);
    }
    ...
    // Keep external events for API polling (limit to last 1000 to prevent memory issues)
    if (this.externalEvents.length >= 1000) {
      this.externalEvents = this.externalEvents.slice(-1000);
    }
  ...
  buildSnapshots(spectators = []) {
    const tag = (list, entryOf) => list.map((o) => {
      const pos = o.pos;
      return { x: pos.x, y: pos.y, z: pos.z, entry: entryOf(o) };
    });
    ...
    const mobEntries = tag(this.mobs.mobs, (m) => ({
      ...
      x: r2(m.pos.x), y: r2(m.pos.y), z: r2(m.pos.z), 
      facing: r2(m.facing), pitch: r2(m.pitch), 
      hp: r2(m.hp), maxHp: r2(m.maxHp), 
      ...(m.flight ? { loaded: m.loaded } : {}),
    }));
    ...
    for (const p of this.players.values()) {
      ...
      if (p.invDirty) {
        ...
        snap.inventory = this.inventoryCache.get(cacheKey);
        p.invDirty = false;
      }
      out.set(p.id, snap);
    }
