...
import { sessionFromCookie, parseCookies } from './auth.js';
...
// Optional: a shared password that grants admin access without needing to be
// logged in with an allow-listed Discord account. Leave ADMIN_PASSWORD unset
// to disable this login path entirely.
...
// Accepts either an allow-listed Discord account (adminIds) or a valid
// password-login cookie (admtok) — either is sufficient.
function adminIdentity(req) {
  const accountId = sessionFromCookie(req.headers.cookie);
  if (accountId != null && adminIds.size > 0) {
    const account = getAccountFull(accountId);
    if (account && adminIds.has(account.discord_id)) return { via: 'discord', username: account.username };
  }
  const cookies = parseCookies(req.headers.cookie);
  if (ADMIN_PASSWORD && verifyAdminToken(cookies.admtok)) {
    return { via: 'password', username: 'admin' };
  }
  return null;
}
...
export async function handleAdmin(req, res) {
  ...
  const adminConfigured = adminIds.size > 0 || !!ADMIN_PASSWORD;

  // Password login/logout are handled before the auth gate below, since
  // logging in is how you get past that gate in the first place.
  ...
  if (url.pathname === '/admin/session' && req.method === 'GET') {
    json(res, 200, { admin: true, username: admin.username });
    return true;
  }
  ...
  if (url.pathname === '/admin/mob-catalog' && req.method === 'GET') {
    json(res, 200, { mobs: Object.entries(MOB_TYPES).map(([id, mob]) => ({ id, name: mob.name })) });
    return true;
  }
  ...
  if (accountMatch && req.method === 'POST') {
    ...
    console.log(`[admin] ${admin.username} [${admin.via}] edited account #${id} (${account.username})`);
