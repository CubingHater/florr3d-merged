// Must be the very first import: ES module imports below run their
// top-level code (including reads like `process.env.ADMIN_PASSWORD` in
// admin.js) before anything later in *this* file runs. Loading dotenv here,
// first, guarantees process.env is populated from .env before any other
// module's top-level code executes and reads it.
...
import { handleExternalApi } from './externalApi.js';
import { handleLeaderboardApi } from './leaderboards.js';
...
async function handleRequest(req, res) {
  ...
  // Handle external API requests for Discord bots
  if (req.url.startsWith('/api/external')) {
    await handleExternalApi(req, res);
    return;
  }
  ...
  if (await handleLeaderboardApi(req, res, requestUrl, accountId)) return;
  ...
  if (pathname === '/join-token') {
    ...
    if (!discordLinked && turnstileConfigured() && !verifyHumanCookie(parseCookies(req.headers.cookie).human, ip)) {
      ...
      const sameSite = (process.env.FRONTEND_URL || '').trim() ? 'SameSite=None' : 'SameSite=Lax';
      res.appendHeader('Set-Cookie',
        `human=${makeHumanCookie(ip)}; Path=/; Max-Age=${HUMAN_TTL_MS / 1000}; HttpOnly; Secure; ${sameSite}`);
    }
    res.writeHead(200, { 'content-type': 'application/json', 'cache-control': 'no-store' });
