...
import { addLeaderboardStat, loadSave, writeSave, removePersistedEternalPharCrowns } from './db.js';
import { clientIp } from './utils.js';
import { petalStat } from '../shared/petals.js';
import { setWorldInstance } from './externalApi.js';
...
// accountId -> live Player instance, for connected accounts only. Read by the
// admin panel (server/admin.js) so inventory edits apply instantly to online
// players instead of only landing on their next save/reconnect.
...
function flushLeaderboard(player) {
  ...
  const fields = {
    ...
    killPoints: 'kill_points', dropPoints: 'drop_points',
  };
  for (const [key, column] of Object.entries(fields)) {
    const value = player.leaderboard[key] || 0;
    if (value > 0) addLeaderboardStat(player.accountId, column, value);
    player.leaderboard[key] = 0;
  }
}
...
export function attachGameServer(httpServer, path = '/ws') {
  ...
  const world = new World({ removePersistedEternalPharCrowns });
  ...
  wss.on('connection', (ws, req) => {
    ...
    ws.on('message', (data, isBinary) => {
      ...
      // Early validation for all messages to prevent processing invalid data
      ...
      if (!player) {
        if (msg?.t === 'join') {
          ...
          if (typeof msg.name !== 'string' && msg.name !== undefined) {
            console.log(`[invalid] refused invalid name type ip=${ip}`);
            return;
          }
          ...
          if (accountId != null && ![...accounts.values()].includes(accountId)) {
            ...
            liveAccounts.set(accountId, player);
          }
