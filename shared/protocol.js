import { VIEW_RADIUS } from './config.js';
import { PETAL_TYPES, petalStat } from './petals.js';
...
const POS = 64;
const ANG = 8192;
const EV = { flash: 0, dmg: 1, toast: 2, chat: 3, pop: 4, craft: 5 };
const EV_NAMES = ['flash', 'dmg', 'toast', 'chat', 'pop', 'craft'];
...
function readPlayerStatic(r) {
  const s = {
    name: r.str(), model: r.str(), level: r.u16(), maxHp: r.f32(),
    rotFactor: r.frac8() * 0.7 + 0.3,
    primary: [], secondary: [], comp: [],
  };
  const slots = r.u8();
  for (let i = 0; i < slots; i++) s.primary.push(readSlot(r));
  for (let i = 0; i < slots; i++) s.secondary.push(readSlot(r));
  const n = r.u8();
  for (let i = 0; i < n; i++) {
...
function writeMob(w, m) {
  ...
  w.u8(m.rarity);
  w.pos(m.pos.x); w.pos(m.pos.y); w.pos(m.pos.z); 
  w.ang(m.facing);
  w.ang(m.pitch);
...
function writeEvent(w, ev) {
  ...
  else if (ev.e === 'craft') {  w.u32(ev.id); w.u8(ev.g); w.u8(ev.l); }
  else w.str(ev.text);
}
function readEvent(r) {
  ...
  if (e === 'craft') return { e, id: r.u32(), g: r.u8(), l: r.u8() };
  return { e, text: r.str() };
}

const CMD_NAMES = ['join', 'input', 'swapSlot', 'swapRows', 'rotSpeed', 'equip', 'chat', 'setModel', 'craftAtt'];
...
export function encodeCmd(msg) {
  ...
  switch (msg.t) {
    ...
    case 'setModel':
      ...
      break;
    case 'craftAtt': {
      // WebSocket callers send explicit fields, while a few older/local
      // callers still use `key`. Supporting both prevents an undefined petal
      // from silently reaching World.handle and leaving the craft animation
      // waiting for a result that can never arrive.
      const [keyType, keyRarity, keyCount] = String(msg.key || '').split(':');
      const type = msg.type ?? keyType;
      const rarity = msg.rarity ?? keyRarity;
      const count = msg.count ?? keyCount;
      w.u8(PETAL_IDX.get(type) ?? 255);
      w.u8(Number(rarity) & 255);
      w.u8(Number(count) & 255);
      break;
    }
    default:
...
export function decodeCmd(buffer) {
  ...
  switch (t) {
    ...
    case 'chat':
      return { t, text: r.str() };
    case 'setModel':
      return { t, model: r.str() };
    case 'craftAtt': {
      const type = PETAL_IDS[r.u8()];
      const rarity = r.u8();
      const count = r.u8();
      return { t, type, rarity, count };
    }
    default:
  ...
  apply(buffer) {
    ...
    this._applySection(r, this.players, () => {
      ...
      if (hasStatic) {
        ...
        p.name = st.name; p.model = st.model; p.level = st.level; p.maxHp = st.maxHp;
