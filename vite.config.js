...
const attachAdmin = (server) => server.middlewares.use((req, res, next) => {
  handleAdmin(req, res).then((handled) => { if (!handled) next(); }, next);
});
...
const attach = (server) => {
  ...
  attachGameServer(server.httpServer); attachAuth(server); attachSession(server); attachAdmin(server); attachMap(server); attachJoinToken(server);
};
